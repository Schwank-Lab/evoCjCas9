...
def importfiles(shortname, protolookup, uniquebarcodelookup):
    ...
    with gzip.open(path+filename, "rt") as fasta_file:
        ...
        for seq_record in SeqIO.parse(fasta_file, 'fastq'):
            barcode = str(seq_record.seq)[3:10]  # barcode is only the last 6 bases of the 3trim sequence
            # print(barcode)
...
cjlibrary_templatedf = pd.read_csv('CjCas9_OFFLib.csv')
...
# analyse 10bp up and 10bp downstream of cut-site:
cjlibrary_templatedf['target'] = cjlibrary_templatedf.apply(lambda x: str(Seq(x.PAM_rv+x.target_rv).reverse_complement()[10+5+2:30-5-2]).upper(),axis=1)
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    with gzip.open(exactpath + "Output/"+shortname+'_3trim.fastq.gz', 'rt') as fastqfile:
        ...
        for seq_record in SeqIO.parse(fastqfile, 'fastq'):
            ...
            target = str(seq_record.seq)[32+5+2:52-5-2] # Target sequence within the sequenced reads
            print('2',target)
            
            libraryseqtarget = templatenumpy[variantindex,libtargetnr]
            print('1',libraryseqtarget)
            # print()
    ...
    cjlibrary_templatedf.to_csv('AnalysisFiles/20221011_6bpwindow_'+shortname+'.csv')
    ...
    with open('20220929_Recombination_Analysis.txt','a') as fp:  # save recombination info in txt file
        fp.write(shortname+"\n")
        fp.write('Total matched targets: {}\n'.format(len(final_diseasedf['match'].unique())))
        fp.write('Total reads: {}\n'.format(len(diseasedf)))
        fp.write('Successful assignment: {}\n'.format(len(final_diseasedf)))
        fp.write('Recombined: {}\n'.format(diseasedf.recombination.sum()))
        fp.write('Percentage recombined: {}\n\n'.format(round((diseasedf.recombination.sum()/(diseasedf.recombination.sum()+len(final_diseasedf)))*100,2)))
