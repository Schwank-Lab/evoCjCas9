...
for  filename in *R1.fastq.gz; do
	shortname="${filename:22:-18}";
	cutadapt -j 0 -e 0 -g GAAACACC -o "Output/"$shortname"_5trim.fastq.gz" $filename >> $datum"_cutadaptlog.txt"
...
for  filename in *R2.fastq.gz; do
	...
	cutadapt -j 0 -e 0 -a GTGACTCCTA --discard-untrimmed -o "Output/"$shortname"_3trim.fastq.gz" $revcompname >> $datum"_cutadaptlog.txt"
done