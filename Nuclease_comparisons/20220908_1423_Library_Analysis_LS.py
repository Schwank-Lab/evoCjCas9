...
salibrary_templatedf = pd.read_csv('SaCas9_o1423.csv')
salibrary_templatedf['spacer_part'] = salibrary_templatedf['spacer'].apply(lambda x: x[:7])
salibrary_templatedf['Spacer_Length'] = salibrary_templatedf['spacer'].apply(lambda x: len(x))
salibrary_templatedf['target'] = salibrary_templatedf['target_rv'].apply(lambda x: str(Seq(x).reverse_complement()))
salibrary_templatedf['Barcode'] = salibrary_templatedf['Barcode'].apply(lambda x: str(Seq(x).reverse_complement()))
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    print(editorname)
    prototemplate = salibrary_templatedf['spacer_part'].values.flatten()
    uniquebarcodetemplate =salibrary_templatedf['Barcode'].values.flatten()
    ...
    salibrary_templatedf["uneditedcount"] = 0
    salibrary_templatedf["indelcount"] = 0
    salibrary_templatedf["totalreads"] = 0

    templatenumpy = salibrary_templatedf.to_numpy()
    # define column position for used column in numpy array:
    uneditedcountnr = salibrary_templatedf.columns.get_loc("uneditedcount")
    editedcountnr = salibrary_templatedf.columns.get_loc("indelcount")
    libtargetnr = salibrary_templatedf.columns.get_loc("target")
    namenr = salibrary_templatedf.columns.get_loc("Identifier")
    ...
    salibrary_templatedf = pd.DataFrame(data = templatenumpy, 
                      index = salibrary_templatedf.index.tolist(), 
                      columns = salibrary_templatedf.columns.tolist())
    
    totalreads = salibrary_templatedf["uneditedcount"] +  salibrary_templatedf["indelcount"]
    salibrary_templatedf['totalreads'] = totalreads
    salibrary_templatedf['totalreads'].replace(0, np.nan, inplace=True)
    
    #store final dataframe containing sequence analysis
    salibrary_templatedf.to_csv('AnalysisFiles/20220909_'+shortname+'.csv')
