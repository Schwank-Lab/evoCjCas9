...
cjlibrary_templatedf['spacer_part'] = cjlibrary_templatedf['Protospacer-Sequence'].apply(lambda x: x[:7])
cjlibrary_templatedf['Spacer_Length'] = cjlibrary_templatedf['Protospacer-Sequence'].apply(lambda x: len(x))
# cjlibrary_templatedf['target'] = cjlibrary_templatedf['target_rv'].apply(lambda x: str(Seq(x).reverse_complement()))
cjlibrary_templatedf['target'] = cjlibrary_templatedf['Target-SequenceReady'].apply(lambda x: str(Seq(x[-39:-19]).reverse_complement()))

cjlibrary_templatedf['Barcode'] = cjlibrary_templatedf['6nt barcode'].apply(lambda x: str(Seq(x).reverse_complement()))
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    cjlibrary_templatedf['variantindex'] = cjlibrary_templatedf.index
    ...
    namenr = cjlibrary_templatedf.columns.get_loc("variantindex")
    ...
    with gzip.open(exactpath + "Output/"+shortname+'_3trim.fastq.gz', 'rt') as fastqfile:
        ...
        for seq_record in SeqIO.parse(fastqfile, 'fastq'):
            ...
            target = str(seq_record.seq)[25:45] # Target sequence within the sequenced reads
            # print(target)
    ...
    cjlibrary_templatedf.to_csv('AnalysisFiles/20221004_'+shortname+'.csv')
