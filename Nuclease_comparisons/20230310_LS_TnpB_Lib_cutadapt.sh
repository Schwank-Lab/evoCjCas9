...
for filename in *R1_001.fastq.gz; do
	...
	cutadapt -j 0 -g tcgtgtgaggttcaa --discard-untrimmed -o "Output/"$shortname"_5trim.fastq.gz" $filename >> $datum"_cutadaptlog.txt"
	cutadapt -j 0 -a ggccggcatggt -m 20 -M 20 --discard-untrimmed -o "Output/"$shortname"_omegaRNA.fastq.gz" "Output/"$shortname"_5trim.fastq.gz" >> $datum"_cutadaptlog.txt"
