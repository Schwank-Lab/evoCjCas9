...
splibrary_templatedf = pd.read_csv('SpCas9_o1422.csv')
splibrary_templatedf['spacer_part'] = splibrary_templatedf['spacer'].apply(lambda x: x[:7])
splibrary_templatedf['Spacer_Length'] = splibrary_templatedf['spacer'].apply(lambda x: len(x))
splibrary_templatedf['target'] = splibrary_templatedf['target_rv'].apply(lambda x: str(Seq(x).reverse_complement()))
splibrary_templatedf['Barcode'] = splibrary_templatedf['Barcode'].apply(lambda x: str(Seq(x).reverse_complement()))
...
for filename in filelist:  # loop through all SAMPLES in a directory with "protospacer" in the name (listfiles1 function)
    ...
    print(editorname)
    prototemplate = splibrary_templatedf['spacer_part'].values.flatten()
    uniquebarcodetemplate =splibrary_templatedf['Barcode'].values.flatten()
    ...
    splibrary_templatedf["uneditedcount"] = 0
    splibrary_templatedf["indelcount"] = 0
    splibrary_templatedf["totalreads"] = 0

    templatenumpy = splibrary_templatedf.to_numpy()
    # define column position for used column in numpy array:
    uneditedcountnr = splibrary_templatedf.columns.get_loc("uneditedcount")
    editedcountnr = splibrary_templatedf.columns.get_loc("indelcount")
    libtargetnr = splibrary_templatedf.columns.get_loc("target")
    namenr = splibrary_templatedf.columns.get_loc("Identifier")
    ...
    splibrary_templatedf = pd.DataFrame(data = templatenumpy, 
                      index = splibrary_templatedf.index.tolist(), 
                      columns = splibrary_templatedf.columns.tolist())
    
    totalreads = splibrary_templatedf["uneditedcount"] +  splibrary_templatedf["indelcount"]
    splibrary_templatedf['totalreads'] = totalreads
    splibrary_templatedf['totalreads'].replace(0, np.nan, inplace=True)
    
    #store final dataframe containing sequence analysis
    splibrary_templatedf.to_csv('AnalysisFiles/20220909_'+shortname+'.csv')
